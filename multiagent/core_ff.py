...
class World(object):
    def __init__(self):
        ...
        # random seed for script agent
        self.script_agents_seed = 0
        self.random_walk_buffer = [0, 0, np.zeros(self.dim_p)] # curr step, total step, acc
    ...
    def step(self):
        ...
        p_force = [None] * len(self.entities) # initialize force for all entities, not only agents
        # apply agent physical controls
        p_force = self.apply_action_force(p_force)
        # apply environment forces
        p_force = self.apply_environment_force(p_force)
        # integrate physical state
        self.integrate_state(p_force) # update positions
    ...
    def apply_action_force(self, p_force):
        ...
        for i, agent in enumerate(self.agents):
            if agent.movable:
                ...
                p_force[i] = (agent.mass * agent.accel if agent.accel is not None else agent.mass) * agent.action.u + noise # agent.action.u is scaled by sensitivity?
        return p_force
    ...
    def integrate_state(self, p_force):
        for i,entity in enumerate(self.entities):
            ...
            for wall in self.walls:
                is_collision = self.check_wall_collision(entity, wall)
                if is_collision:
                    if wall.orient == 'H':
                        entity.state.p_vel[1] = -np.sign(wall.axis_pos) * np.abs(entity.state.p_vel[1])
                    else:
                        entity.state.p_vel[0] = -np.sign(wall.axis_pos) * np.abs(entity.state.p_vel[0])
                    break
    ...
    def get_entity_collision_force(self, ia, ib):
        ...
        if self.cache_dists:
            ...
            delta_pos = entity_a.state.p_pos - entity_b.state.p_pos
            dist = np.sqrt(np.sum(np.square(delta_pos)) + 1e-6)
    ...
    def get_wall_collision_force(self, entity, wall):
        ...
        if (ent_pos[prll_dim] < wall.endpoints[0] - entity.size or
            ...
            return None  # entity is beyond endpoints of wall, no need to apply any force
    ...
    def check_wall_collision(self, entity, wall):
        if entity.ghost and not wall.hard:
            return None  # ghost passes through soft walls
        if wall.orient == 'H':
            prll_dim = 0
            ...
            perp_dim = 0
        ent_pos = entity.state.p_pos
        if (ent_pos[prll_dim] > wall.endpoints[0] and ent_pos[prll_dim] < wall.endpoints[1] and np.abs(ent_pos[perp_dim] - wall.axis_pos) < entity.size):
            return True  # entity collides with the wall
        else:
            return False # entity doesn't collide with the wall
